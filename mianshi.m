...
十、设计模式
 ...
 十二、关于自己的问题
           1.紫金山的架构
                    mvc、mvvm框架思想，六大设计原则和几个思想、内容上分业务，功能，基础
           2.如何重构？
                 1.排查block和timer的循环引用，delloc能否正常释放（检测工具MLeaksFinder），kvo和通知是否释放得当
                 2.去除masory布局警告，整理资源文件，删除未使用的图片、资源、pod库，更换xib为原代码
                 3.优化启动时间
                 4.引入fps检测类，在用的过程中对fps过低的页面进行优化
                 5.性能优化
                 6.去除冗余代码
           3.紫金山用到的技术栈
           4.工作中遇到的问题，如何解决
           5.容易忘记的问题
                 static:修饰局部变量时，只会初始化一次且在程序中只有一份内存
                         不可以改变局部变量的作用域,但可延长局部变量的生命周期（直到程序结束才销毁
                         当static关键字修饰全局变量时，作用域仅限于当前文件，外部类是不可以访问到该全局变量的
                 如何处理本地数据安全的？
                     沙盒不安全，用KeyChain存储
                 如何设计一个方案去应对后段频繁更改的字段接口？
